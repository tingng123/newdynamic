...
public class CharacterController2D : MonoBehaviour
{
	...
	public float m_MovementSmoothing = .01f;  // How much to smooth out the movement
	...
	public float k_CeilingRadius = .02f; // Radius of the overlap circle to determine if the player can stand up
	private Rigidbody2D m_Rigidbody2D;
	private bool m_FacingRight = true;  // For determining which way the player is currently facing.
	private Vector3 m_Velocity = Vector3.zero;

	public Collider2D standColli;
	public Collider2D couchColli;
	...
	private void Awake()
	{
		...
		standColli = gameObject.GetComponent<BoxCollider2D>();
		couchColli = gameObject.GetComponent<CapsuleCollider2D>();
	...
	public void Move(float move, bool crouch, bool jump)
	{
		...
		if (m_Grounded || m_AirControl)
		{
			...
			if (crouch)
			{
				if (!m_wasCrouching)
				{
					...
					couchColli.enabled = true;
					standColli.enabled = false;
				}
			...
			else
			{
				...
				if (m_wasCrouching)
				{
					...
					standColli.enabled = true;
					couchColli.enabled = false;
				}
			}
		...
		if (m_Grounded && jump && m_wasCrouching == false)
		{
